...
Process:
1. Propose ~8 candidate destinations from your own knowledge, biased to mood, interests, climate, origin. Make the pool SPAN THE BUDGET RANGE: when the budget is generous, deliberately include several higher-end and farther-afield options (not only cheap short hops from the origin) so a richer trip can actually win. Favor well-known destinations likely to have data.
...
6. Treat budget as a comfortable TARGET BAND (~70%), not a floor or ceiling. A bigger budget signals appetite for a richer trip, so LEAD the top ✓ Recommended pick with the richest well-fitting, safe, in-budget candidate whose total lands near 60-80% of budget. Do NOT pad a trip just to spend, and NEVER exceed budget. If nothing reaches the band (e.g. a short trip near a large budget, where even pricey options fall short), lead with the RICHEST strong-fit option (closest to the band from below), not the cheapest, and note in "why" that the budget comfortably exceeds the trip's cost (a longer stay or higher tier would use more). Diversify the rest: include at least one lower-cost value option so the traveler sees the trade-off, and you MAY mark one near-ceiling aspirational pick (>=~85% of budget) "premium":true, noting in "why" it's the high-end option the budget unlocks. Premium picks must still be safe and within budget.
